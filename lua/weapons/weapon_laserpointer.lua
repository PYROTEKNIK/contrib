...
function SWEP:DrawBeam(ply, origin, dir, color, phase, startoverride)
    ...
    if (beammode) then
        ...
        -- only the first beam is rendered in low beam mode, only a certain distance
        if (phase == 0) then
...
function SWEP:MakePlane(pos, targetpos)
    ...
    -- Function that will be called whenever collision happends
...
function SWEP:PostDrawViewModel(vm, weapon, ply)
    ...
    draw.SimpleText(pval .. psuf, font, 0, 9, indcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
function SWEP:DrawWorldModel(query)
    ...
    if (query == true) then return mrt end
end
